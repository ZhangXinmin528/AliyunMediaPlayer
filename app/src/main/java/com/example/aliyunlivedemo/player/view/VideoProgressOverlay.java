package com.example.aliyunlivedemo.player.view;
...
public class VideoProgressOverlay extends FrameLayout {
    ...
    public void show(int delProgress, int curPosition, int duration) {
        ...
        if (mSeekDialogStartProgress == -1) {
            Log.i("DDD", "show: _resumePlayVideo seek = " + mSeekDialogStartProgress);
        ...
        if (delProgress > 0) {
            // 回退
            mSeekIcon.setImageResource(R.drawable.ic_player_video_back);
        } else {
            // 前进
            mSeekIcon.setImageResource(R.drawable.ic_player_video_speed);
        }
